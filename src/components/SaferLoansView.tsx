...
interface SaferLoansViewProps {
  isDark: boolean;
  onBackToLiquidity: () => void;
}

interface SaferLoanItem {
  id: string;
  name: string;
  unitPrice: string;
  unitsHeld: string;
...
const SAFER_LOANS: SaferLoanItem[] = [
  {
    id: 'sl-1',
    name: 'US Treasury Benchmark Note 10-Year',
    unitPrice: '$98.40',
    unitsHeld: '3,100',
    currentRate: '4.35%',
    totalValue: '$305,040',
    issuer: 'United States Department of the Treasury',
    maturity: 'August 2034',
    rating: 'AAA / Aaa Sovereign',
    coupon: '4.25% Semi-Annual',
  },
  {
    id: 'sl-2',
    name: 'UK Sovereign Gilt Due 2034',
    unitPrice: '$101.20',
    unitsHeld: '2,200',
    currentRate: '4.25%',
    totalValue: '$222,640',
    issuer: 'HM Treasury / UK Debt Management Office',
    maturity: 'January 2034',
    rating: 'AA / Aa3 Sovereign',
    coupon: '4.125% Semi-Annual',
  },
  {
    id: 'sl-3',
    ...
    rating: 'AAA Supranational',
    coupon: '3.75% Annual',
  },
  {
    id: 'sl-4',
    name: 'World Bank (IBRD) Sustainable Note',
    unitPrice: '$100.50',
    unitsHeld: '1,030',
    currentRate: '4.10%',
    totalValue: '$103,504',
...
export const SaferLoansView: React.FC<SaferLoansViewProps> = ({
  isDark,
  onBackToLiquidity,
}) => {
  ...
  const exportCSV = () => {
    ...
    const rows = SAFER_LOANS.map((loan) => [
      `"${loan.name}"`,
      `"${loan.unitPrice}"`,
      `"${loan.unitsHeld}"`,
      `"${loan.currentRate}"`,
    ...
    link.setAttribute('download', 'safer_loans_holdings.csv');
  ...
  return (
    <div className="flex flex-col gap-5 sm:gap-6 w-full animate-in fade-in duration-200">
      ...
      {/* Safer Loans Table matching screenshot */}
      <div
        id="safer-loans-table-card"
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse min-w-[720px]">
            ...
            <tbody className="text-xs sm:text-sm">
              {SAFER_LOANS.map((row, index) => {
                const isExpanded = expandedIds.has(row.id);

                return (
                  <React.Fragment key={row.id}>
                    <tr
                      id={`safer-loan-row-${row.id}`}
                      onClick={() => toggleRow(row.id)}
                      className={`group transition-all duration-150 cursor-pointer select-none ${
                        index !== SAFER_LOANS.length - 1 || isExpanded
                          ? isDark
                      ...
                      >
                        <td colSpan={6} className="py-3 px-4 sm:px-6 text-xs animate-in fade-in duration-150">
                          <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-slate-500 dark:text-slate-400">
                            ...
                            <div>
                              <span className="font-medium text-slate-700 dark:text-slate-300">Coupon Rate: </span>
                              {row.coupon}
                            </div>
                          </div>
                        </td>
