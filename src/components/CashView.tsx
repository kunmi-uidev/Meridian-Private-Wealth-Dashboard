...
const CASH_HOLDINGS: CashItem[] = [
  {
    id: 'cash-1',
    name: 'J.P. Morgan US Treasury Liquidity Fund',
    unitPrice: '$1.00',
    unitsHeld: '340,000',
    currentRate: '5.25%',
    totalValue: '$340,000',
    fundType: 'Government Money Market Fund (AAAmmf)',
    settlement: 'T+0 Same Day Liquidity',
    currency: 'USD',
    custodian: 'JPMorgan Chase Bank, N.A.',
  },
  {
    id: 'cash-2',
    name: 'BlackRock ICS USD Premier Liquidity',
    unitPrice: '$1.00',
    unitsHeld: '215,000',
    currentRate: '5.18%',
    totalValue: '$215,000',
    fundType: 'Institutional Cash Series (Prime/Treasury)',
    settlement: 'T+0 Immediate Sweep',
    currency: 'USD',
    custodian: 'BlackRock Institutional Trust Company',
  },
  {
    id: 'cash-3',
    name: 'Sprott Physical Gold Trust Hedges',
    unitPrice: '$18.40',
    unitsHeld: '4,783',
    currentRate: '12.8%',
    totalValue: '$88,016',
    fundType: 'Fully Allocated Physical Bullion (PHYS)',
    settlement: 'T+1 Physical Delivery / Redemption',
    currency: 'USD',
...
export const CashView: React.FC<CashViewProps> = ({
  ...
  const exportCSV = () => {
    ...
    const rows = CASH_HOLDINGS.map((cash) => [
      `"${cash.name}"`,
      `"${cash.unitPrice}"`,
      `"${cash.unitsHeld}"`,
      `"${cash.currentRate}"`,
    ...
    link.setAttribute('download', 'cash_holdings.csv');
  ...
  return (
    <div className="flex flex-col gap-5 sm:gap-6 w-full animate-in fade-in duration-200">
      ...
      {/* Cash Holdings Table matching screenshot */}
      <div
        id="cash-table-card"
                  ...
                  <React.Fragment key={row.id}>
                    <tr
                      ...
                      className={`group transition-all duration-150 cursor-pointer select-none ${
                        index !== CASH_HOLDINGS.length - 1 || isExpanded
                          ? isDark
                      ...
                      >
                        <td colSpan={6} className="py-3 px-4 sm:px-6 text-xs animate-in fade-in duration-150">
                          <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-slate-500 dark:text-slate-400">
                            ...
                            <div>
                              <span className="font-medium text-slate-700 dark:text-slate-300">Settlement: </span>
                              {row.settlement}
                            </div>
                            <div>
                            ...
                            <div>
                              <span className="font-medium text-slate-700 dark:text-slate-300">Custodian: </span>
                              {row.custodian}
                            </div>
                          </div>
                        </td>
